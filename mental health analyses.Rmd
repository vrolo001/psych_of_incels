...
dfincels <- incel_cleaned_data #data already imported manually and we're just renaming it
...
```{r}
# create list of constructs and items that comprise them

var_list <- list("NFR" = c(paste0("NFR_", 1:6)),
                 "ME" = c(paste0("ME_", 1:6)),
                 "LoE" = c(paste0("LoE_", 1:6)),
                 "RUM" = c(paste0("RUM_", 1:4)),
                 "NFR" = c(paste0("NFR_", 1:4)),
                 "MVS" = c(paste0("MVS_", 1:2), "MVS_2_1", "MVS_2_2"), 
                 "PQ9" = c(paste0("NHSdepress_", 1:8)),
                 "GAD" = c(paste0("NHSdepress_", 9:15)),
                 "lone" = c(paste0("lonely_", 1:3)),
                 "SWL" = c(paste0("Q75_", 1:5)),
                 "soides" = c(paste0("soides_", 1:3)))

# function to calculate alpha based on columns in a dataframe

calculate_alpha <- function(cols, df) {
  result <- psych::alpha(select(df %>% filter(sex == 0), all_of(cols)))$total$raw_alpha
  return(result)
}

#calculate_alpha(var_list$NFR, dfincels) #just to test function works properly

# use lapply to iterate over the list and apply the function
alphas <- lapply(var_list, calculate_alpha, dfincels)

data.frame(Variable = names(alphas),
           Alpha = unlist(alphas))
...
```{r}
#create subset used for rest of mental health analyses
...
incels_mh <- dfincels %>%
  dplyr::select(., (c(incel, sex, age, soides, MVS, 
                      ...
                      ethnic_1:ethnic_7, sexorien, gender, country, 
                      politic, porn, forums, incel4life, blackpill, forumWellbeing, femopinion, feminist))) %>% #for exploratory
  filter(sex == 0 & incel == 0 | sex == 0 & incel == 1)

# scale variables for confirmatory hypotheses

vars_scale <- c("age", "soides", "MVS", 
                "PHQ9_mean", "GAD_mean", "lonely_mean", "SWL_mean", 
                "PHQ9_all", "GAD_all", "lonely_all", "SWL_all",
                "TIV_NFR", "TIV_ME", "TIV_LoE", "TIV_RUM", "TIV")

for(var in vars_scale){
  incels_mh <- incels_mh %>%
    group_by(incel) %>%
    mutate(!!paste0(var, "_Z") := c(scale(!!sym(var)))) %>%
    ungroup()
}
...
for (vars in vars_scale){
  scale_names <- c(scale_names, paste0(vars, "_Z"))
}

# create function that adds the total number of outliers(defined as > 3.30 or < -3.30)

out_sum <- function(col, df){
  result <- sum(df[[col]] > 3.30 | df[[col]] < -3.30, na.rm = TRUE)
  return(result)
}

#out_sum("age_Z", incels_mh) #test function works
sapply(scale_names, out_sum, incels_mh)
...
```{r}
incels_age <- incels_mh %>%
  filter(age_Z >= -3.30 & age_Z <= 3.30)

favstats(~ age, incel, data = incels_age)

t.test(age ~ incel, data = incels_age, var.equal = 
         ifelse((leveneTest(age ~ as.factor(incel), data = incels_age))$`Pr(>F)` > .05, TRUE, FALSE)[1]) 
cohens_d(age ~ incel, data = incels_age)

set.seed(1993)
age.boot <- Boot(lm(age ~ incel, data = incels_age), f = coef, R = 5000)
...
data_nonincels <- data_mh %>%
  filter(incel == 0)

rcorr(as.matrix(data_incels[, c("TIV", "PHQ9_mean", "GAD_mean", "lonely_mean", "SWL_mean")], type = "pearson"))
rcorr(as.matrix(data_nonincels[, c("TIV", "PHQ9_mean", "GAD_mean", "lonely_mean", "SWL_mean")], type = "pearson"))

rcorr(as.matrix(data_incels[, c("soides", "PHQ9_mean", "GAD_mean", "lonely_mean", "SWL_mean")], type = "pearson"))
rcorr(as.matrix(data_nonincels[, c("soides", "PHQ9_mean", "GAD_mean", "lonely_mean", "SWL_mean")], type = "pearson"))

data_list <- list(data_incels = as.data.frame(data_incels),
                 data_nonincels = as.data.frame(data_nonincels))
################################### Fisher's Z ###################################
library(cocor)
library(lsr)

#TIV
cocor(formula = ~ TIV + PHQ9_mean | TIV + PHQ9_mean, 
      data = data_list)

cocor(formula = ~ TIV + GAD_mean | TIV + GAD_mean, 
      data = data_list)

cocor(formula = ~ TIV + lonely_mean | TIV + lonely_mean, 
      data = data_list)

cocor(formula = ~ TIV + SWL_mean | TIV + SWL_mean, 
      data = data_list)

#SOI desire
cocor(formula = ~ soides + PHQ9_mean | soides + PHQ9_mean, 
      data = data_list)

cocor(formula = ~ soides + GAD_mean | soides + GAD_mean, 
      data = data_list)

cocor(formula = ~ soides + lonely_mean | soides + lonely_mean, 
      data = data_list)

cocor(formula = ~ soides + SWL_mean | soides + SWL_mean, 
      data = data_list)
```
...
data_exp <- data %>%
  ...
  mutate(mental = `M Health`, physical = `P Health`) %>%
  filter(sex == 0 & incel == 0 | sex == 0 & incel == 1)
...
```{r}
data_love <- data_exp %>%
  filter(relation == 1 & seekrelation < 6) %>% #all the single gentlemen
...
## Ethnic breakdowns (whole sample and by incel self-identification)
...
#Proportion of white and BIPOC by incel self identification
data_mh <- data_mh %>%
  mutate(ethnic_di = ifelse(ethnicity == 1, 0, 1))

data_mh$ethnic_di <- factor(data_mh$ethnic_di, levels = c('0', '1'),
                              labels = c('White','BIPOC'))

tblethnic <- xtabs(~ incel + ethnic_di, data = data_mh)
rowethnic <- prop.table(tblethnic, 1)
colethnic <- prop.table(tblethnic, 2)
chisq.test(tblethnic)

padjethnic <-.05/(nrow(tblethnic)*ncol(tblethnic)) #new alpha value = .0125
critethnic <- qnorm(padjethnic/2) #new critical value of |2.50|

ethnicexp <- chisq.test(tblethnic)$expected
ethnicres <- chisq.test(tblethnic)$stdres

```

## Gender identity (whole sample and by incel-identification)

```{r}
data_mh$gender_cat <- factor(data_mh$gender, levels = c("0", "1", "2", "3", "4", "5", "6"),
                             labels = c("man", "woman", "non-binary", "prefer not to say", "other", "trans male", "trans female"))

summary(data_mh$gender_cat)/nrow(data_mh)*100 #breakdown by gender identity
prop.table(xtabs(~ incel + gender_cat, data = data_mh), 1)*100 #breakdown by incel and gender identity

data_mh <- data_mh %>%
  mutate(gender_di = ifelse(gender == 0, 0, 
                            ifelse(gender == 1 | gender == 2 | gender == 4 | gender == 6, 1, NaN)))

data_mh$gender_di <- factor(data_mh$gender_di, levels = c('0', '1'),
                              labels = c('Cisgender','non-cis'))

tblgender <- xtabs(~ incel + gender_di, data = data_mh)
rowgender <- prop.table(tblgender, 1)*100
chisq.test(tblgender)

```

## Sexual orientation (whole sample and by incel-identification)

```{r}
data_mh$sexorien_cat <- factor(data_mh$sexorien, levels = c ("0", "1", "2", "3", "4", "5", "6", "7"),
                               labels = c("heterosexual", "gay", "lesbian", "bisexual", "pansexual", "questioning", "prefer not to say", "other"))

summary(data_mh$sexorien_cat)/nrow(data_mh)*100 #breakdown by sexual orientation
prop.table(xtabs(~ incel + sexorien_cat, data = data_mh), 1)*100 #breakdown by incel and sexual orientation

data_mh <- data_mh %>%
  mutate(sexorien_di = ifelse(sexorien == 0, 0, 1))
data_mh$sexorien_di <- factor(data_mh$sexorien_di, levels = c('0', '1'),
                              labels = c('Hetero','non-hetero'))


tblsexorien <- xtabs(~ incel + sexorien_di, data = data_mh)
rowsexorien <- prop.table(tblsexorien, 1)
chisq.test(tblsexorien)
...
## Which incels fair better?
  ...
  - How do forum-using incels differ from non-forum users on variables that are typically thought of as incel-typical and/or maladaptive? (t-tests comparing them on political orientation, blackpill belief, incel permanency, and feminism, and porn use)
  
  - Have incels' opinions of women changed as a result of forum use? (one-sample t w/ femopinion) 
  - Are incels who participate in forums or watch more porn more feminist (or likely to identify as one; feminist regressed on porn and forum)?
...
data_incels <- data_mh %>%
  ...
  mutate(incel4life = 6 - incel4life, blackpill = 6 - blackpill, forumWellbeing = 6 - forumWellbeing, #recoding so > values = 
         ...
         femopinionZ = scale(femopinion), feministZ = scale(feminist)) #only 1 outlier for porn
...
incel_feminist.lm <- data_incels %>%
  filter(pornZ >= -3.30 & pornZ <= 3.30 ) %>%
  lm(feminist ~ 1 + porn + forums, data = .)
summary(incel_feminist.lm)

#One sample t-tests

t.test(data_incels$forumWellbeing, mu = 3)
t.test(data_incels$blackpill, mu = 3)
t.test(data_incels$incel4life, mu = 3) #significant
(favstats(~ incel4life, data = data_incels)$mean - 3)/favstats(~ incel4life, data = data_incels)$sd #effect size
t.test(data_incels$femopinion, mu = 3)
(favstats(~ femopinion, data = data_incels)$mean - 3)/favstats(~ femopinion, data = data_incels)$sd

#Group comparisons (forum users vs non users)

data_users <- data_mh %>%
  filter(incel == 1) %>%
  mutate(incel4life = 6 - incel4life, blackpill = 6 - blackpill, feminist = 6 - feminist, #recoding so > values = > scores
        politicZ = scale(politic), incel4lifeZ = scale(incel4life), blackZ = scale(blackpill), feministZ = scale(feminist))

favstats(~ politic, forums, data = data_users)
leveneTest(politic ~ as.factor(forums), data = data_users)$`Pr(>F)` #homogeneity met
t.test(politic ~ forums, data = data_users, var.equal = TRUE) #no political orientation differences

leveneTest(incel4life ~ as.factor(forums), data = data_users)$`Pr(>F)` #homogeneity met
t.test(incel4life ~ forums, data = data_users, var.equal = TRUE) #t = -3.49, df = 133, p < 0.001
favstats(~ incel4life, forums, data = data_users) #forum users had greater beliefs that they'd be incels for life
cohens_d(incel4life ~ forums, data = data_users)
incel4life.boot <- data_users %>%
  filter(!is.na(incel4life), !is.na(forums)) #Boot was originally not working because of NAs and na.omit() wasn't working either
set.seed(1993)
incel4life.boot <-  Boot(lm(incel4life ~ forums, data = incel4life.boot), f = coef, R = 5000)
confint(incel4life.boot, level = .95, type = "norm")

favstats(~ blackpill, forums, data = data_users)
leveneTest(blackpill ~ as.factor(forums), data = data_users)$`Pr(>F)` #homogeneity met
t.test(blackpill ~ forums, data = data_users, var.equal = TRUE) #marginally significant

favstats(~ feminist, forums, data = data_users)
leveneTest(feminist ~ as.factor(forums), data = data_users)$`Pr(>F)` #homogeneity met
t.test(feminist ~ forums, data = data_users, var.equal = TRUE) #marginally significant
...
#Finding percentages of incels who believe, to some degree, in the blackpill and in permanency of inceldom
