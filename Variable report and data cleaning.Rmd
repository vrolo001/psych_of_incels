...
* Height (*height*), where participants could complete different boxes to specify their height in feet (*height_40_TEXT*) and inches (*height_41_TEXT*) or in centimetres (*height_42_TEXT*). A column *height_cm* was created transforming *height_40_TEXT* and *height_41_TEXT* into cms. Then, these values and those in the *height_42_TEXT* column were placed under a single column *height*

```{r}
data <- data %>%
  mutate(height_cm = ((height_40_TEXT*30.48) + (height_41_TEXT*2.54)), 
         height = ifelse(is.na(height_cm), height_42_TEXT, height_cm))
...
* Reasons for being single (*RBS_1* to *RBS_37*, adapted from Apostolou, 2019), participants could select multiple answers for being single, some of which were internal, and others external. An RBS_38 item allowed for an "other" option
...
* Last time having a romantic or sexual encounter (*hadsex*), where 1 = never had a romantic or sexual encounter, 2 = over3 years ago, 3 = over a year ago, 4 = over 6 months ago, 5 = within the last 6 months
...
* Currently seeking a romantic relationship (*seekrelation*), where 1 = definitely yes, 2 = probably yes, 3 = might or might not, 4 = probably not, 5 = definitely not, 6 = no (because I am already in a commited relationship or married)
...
* Belief in being an incel for the rest of their life (*incel4life*), scale where 1 = definitely yes, and 5 = definitely  not

* Subscribing to the blackpill worldview (*blackpill*), scale where 1 = definitely yes, and 5 = definitely  not

* Perceived wellbeing improvement as a result of forum use (*forumWellbeing*, shown if *forums* = 1), scale where 1 = much better and 5 = much worse
